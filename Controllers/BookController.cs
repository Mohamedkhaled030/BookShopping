...
namespace BookShopping.Controllers
{
    ...
    public class BookController : Controller
    {
        ...
        public BookController(IBookRepository bookRepo, IGenreRepository genreRepo, IFileService fileService,
            IStockRepository stockRepository, ApplicationDbContext applicationDbContext)
        {
            _bookRepo = bookRepo;
        ...
        public async Task<IActionResult> TopSellingBooks(DateTime? startDate, DateTime? endDate, int topN = 5)
        {
            if (!startDate.HasValue || !endDate.HasValue)
            {
                startDate = DateTime.UtcNow.AddDays(-30); // افتراضي: آخر 30 يومًا
                endDate = DateTime.UtcNow;
            }

            var books = await _bookRepo.GetTopSellingBooks(startDate.Value, endDate.Value, topN);

            if (books == null || !books.Any())
            {
                ViewBag.Message = "No books found in the given date range.";
                return View(new List<TopSellingBookModel>());
            }
        ...
        public async Task<IActionResult> AddBook(BookDTO bookToAdd)
        {
            ...
            try
            {
                ...
                var stock = await _StockRepository.GetStockById(book.Id);
                var stocks = new Stock();
                if (stock == null)
                {

                    stocks.BookId = book.Id;
                    stocks.Quantity = bookToAdd.Qauntity;


                    await _db.AddAsync(stocks);
